...
```bash
aws lambda create-function --function-name petstoregraal \
...
```bash
aws lambda invoke --function-name petstoregraal --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
